...
class Lp_layer(Layer):
    ...
    def __init__(self, units,
        ...
        super(Lp_layer, self).__init__(**kwargs)
        self.units = units
    ...
    def get_config(self):
        ...
        base_config = super(Lp_layer, self).get_config()
...
class Lp_SGD(Optimizer):     
    def __init__(self,p, learning_rate=0.05, **kwargs):
        learning_rate = kwargs.pop('lr', learning_rate)
        super(Lp_SGD, self).__init__(**kwargs)
        self.p=p
    ...
    def get_updates(self, loss, params):
        ...
        wght=weights[0]
        g=weights[2]
        n_aff_Lp=(K.sum(K.abs(wght)**p,axis=0))**(1/p)
        #n_aff_L2=K.expand_dims(n_aff_L2,axis=1)
        #L2_nm=K.transpose(K.tile(n_aff_L2,784))
        g_delt=K.batch_dot(K.transpose(grmn[0]),K.transpose(K.cast(wght, dtype='float32')),axes=1)/K.expand_dims(n_aff_Lp,axis=1)
        term_1=g/n_aff_Lp
        term_2=g*K.squeeze(g_delt,axis=1)/(n_aff_Lp**p)
        n_grad=term_1*grmn-term_2*(wght**(p-1))
        
        #grads[0]=K.squeeze(n_grad,axis=0)
        #grads[2]=K.squeeze(g_delt,axis=1)
        grads[0]=n_grad
    ...
    def get_config(self):
        ...
        base_config = super(Lp_SGD, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
...
n_aff_Lp=(np.sum(np.abs(veight)**p,0))**(1/p)
Lp_nm=np.tile(n_aff_Lp,[784,1])
weight=g_init*veight/Lp_nm
...
model_nm.add(Lp_layer(units,kernel_constraint=MaskWeights(non_sparse),activation='sigmoid'))
...
for epoch in range(0,epochs):
    model_nm.compile(loss='sparse_categorical_crossentropy', optimizer=Lp_SGD(p,0.05), metrics=['accuracy'])
    ...
    if epoch<epochs: # Add random new weights if not last run
        ...
        n_aff_Lp=(np.sum(np.abs(veight)**p,0))**(1/p)
        Lp_nm=np.tile(n_aff_Lp,[784,1])
        weight=g_vec*veight/Lp_nm
        weight[np.isnan(weight)]=0
        wght_vc[0]=weight     
    
        model_nm = Sequential()
        model_nm.add(Flatten(input_shape=(28,28,1)))
        model_nm.add(Lp_layer(units,kernel_constraint=MaskWeights(wght_ker),activation='sigmoid'))
        model_nm.add(Dense(10, activation='softmax'))
