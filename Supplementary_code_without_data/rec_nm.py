...
n_aff=np.count_nonzero(vh,0)
mrph_nm=np.tile(1/n_aff,[HIDDEN_DIM,1])
wh=s*vh*mrph_nm
wh[np.isnan(wh)]=0
...
def train_one_epoch(in_state,epoch=0,n_rep=0):
    ...
    if n_rep>0:
        ...
        vh.reshape(HIDDEN_DIM**2)[new_inds]=0
        ...
        vh[pos_rws[new_inds],pos_cls[new_inds]]=new_ws
        wh_mask[pos_rws[new_inds],pos_cls[new_inds]]=1
        
        n_aff=np.count_nonzero(vh,0)
        mrph_nm=np.transpose(np.tile(n_aff,[HIDDEN_DIM,1]))
        mrph_nm=np.tile(1/n_aff,[HIDDEN_DIM,1])
        wh=s*vh*mrph_nm
        wh[np.isnan(wh)]=0
    ...
    for ward in range(train_per_epoch):
        ...
        n_aff=np.count_nonzero(wh_mask,0)
        mrph_nm=np.tile(1/n_aff,[HIDDEN_DIM,1])
        dh=s*dh_raw*mrph_nm
        dh[np.isnan(dh)]=0
        
        ds=np.sum(np.dot(vh,dh.T))/s
    ...
    vh[np.abs(wh)<w_thresh]=0
    wh_mask[np.abs(wh)<w_thresh]=0
             
                                
    if in_state['not_last']: # Add random new weights if not last run
        n_con_2=np.count_nonzero(vh)
