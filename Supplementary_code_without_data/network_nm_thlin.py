...
model_nm.add(Morph_layer(units,kernel_constraint=MaskWeights(non_sparse),activation='relu'))
...
for epoch in range(0,epochs):
    ...
    if epoch<epochs: # Add random new weights if not last run
        ...
        model_nm = Sequential()
        model_nm.add(Flatten(input_shape=(28,28,1)))
        model_nm.add(Morph_layer(units,kernel_constraint=MaskWeights(wght_ker),activation='relu'))
        model_nm.add(Dense(10, activation='softmax'))
