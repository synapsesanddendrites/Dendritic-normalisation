...
class L2_L0_layer(Layer):
    ...
    def __init__(self, units,
        ...
        super(L2_L0_layer, self).__init__(**kwargs)
        self.units = units
    ...
    def get_config(self):
        ...
        base_config = super(L2_L0_layer, self).get_config()
...
class L2_L0_SGD(Optimizer):     
    def __init__(self, learning_rate=0.05, **kwargs):
        learning_rate = kwargs.pop('lr', learning_rate)
        super(L2_L0_SGD, self).__init__(**kwargs)
    ...
    def get_updates(self, loss, params):
        ...
        wght=weights[0]
        s=weights[2]
        n_aff_L2=K.sqrt(K.sum(K.square(wght),axis=0))
        
        n_all_L0= K.not_equal(wght, 0.0)
        n_aff_L0=K.sum(K.cast(n_all_L0,'float32'),axis=0)

        g_delt=K.batch_dot(K.transpose(grmn[0]),K.transpose(K.cast(wght, dtype='float32')),axes=1)/(K.expand_dims(n_aff_L2,axis=1)*K.expand_dims(n_aff_L0,axis=1))
        term_1=s/(n_aff_L2)
 
        term_2=s*K.squeeze(g_delt,axis=1)/K.square(n_aff_L2)
        n_grad=(term_1*grmn-term_2*wght)/n_aff_L0
        
        grads[0]=n_grad
        s=K.sum(g_delt)
        grads[2]=K.expand_dims(s*K.ones(1),axis=1)
    ...
    def get_config(self):
        ...
        base_config = super(L2__L0_SGD, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
...
n_aff_L2=np.sqrt(np.sum(np.square(veight),0))
L2_nm=np.tile(n_aff_L2,[784,1])
n_aff_L0=np.count_nonzero(veight,0)
L0_nm=np.tile(n_aff_L0,[784,1])

weight=g_init*veight/(L2_nm*L0_nm)
...
model_nm.add(L2_L0_layer(units,kernel_constraint=MaskWeights(non_sparse),activation='sigmoid'))
model_nm.add(Dense(10, activation='softmax'))


wghtv=model_nm.get_weights()
wghtv[0]=weight
wghtv[2]=g_init*np.zeros(1)
wghtv[3]=fin_wghts
...
for epoch in range(0,epochs):
    model_nm.compile(loss='sparse_categorical_crossentropy', optimizer=L2_L0_SGD(0.05), metrics=['accuracy'])
    ...
    if epoch<epochs: # Add random new weights if not last run
        ...
        n_aff_L0=np.count_nonzero(veight,0)
        L0_nm=np.tile(n_aff_L0,[784,1])
        weight=g_vec*veight/(L2_nm*L0_nm)
        weight[np.isnan(weight)]=0
        wght_vc[0]=weight     
    
        model_nm = Sequential()
        model_nm.add(Flatten(input_shape=(28,28,1)))
        model_nm.add(L2_L0_layer(units,kernel_constraint=MaskWeights(wght_ker),activation='sigmoid'))
        model_nm.add(Dense(10, activation='softmax'))
