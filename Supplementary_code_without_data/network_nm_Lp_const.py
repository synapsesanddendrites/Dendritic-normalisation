...
class Lp_SGD(Optimizer):     
    ...
    def get_updates(self, loss, params):
        ...
        pref=s/n_aff_Lp
        term_1=grmn
        g_temp=K.batch_dot(K.transpose(grmn[0]),K.transpose(K.cast(wght, dtype='float32'))/K.expand_dims(n_aff_Lp**p,axis=1),axes=1)
        g_nm=K.transpose(K.tile(g_temp,784))
        term_2=g_nm*wght**(p-1)
        n_grad=pref*(term_1-term_2)
        s_delt=K.sum(g_temp)/s
