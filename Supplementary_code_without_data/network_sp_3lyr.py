...
iwght[ind_allowed]=vals_allowed
iker[ind_allowed]=ker_allowed
weight_mid=np.reshape(iwght,[units,units])
...
iwght[ind_allowed]=vals_allowed
iker[ind_allowed]=ker_allowed
weight_end=np.reshape(iwght,[units,units])
...
model_sp.add(Dense(units,kernel_constraint=MaskWeights(non_sparse_2),activation='sigmoid'))
model_sp.add(Dense(units,kernel_constraint=MaskWeights(non_sparse_3),activation='sigmoid'))
...
for epoch in range(0,epochs):
    ...
    if epoch<epochs: # Add random new weights if not last run
        ...
        model_sp = Sequential()
        model_sp.add(Flatten(input_shape=(28,28,1)))
        model_sp.add(Dense(units,kernel_constraint=MaskWeights(wght_ker_1),activation='sigmoid'))
        model_sp.add(Dense(units,kernel_constraint=MaskWeights(wght_ker_2),activation='sigmoid'))
        model_sp.add(Dense(units,kernel_constraint=MaskWeights(wght_ker_3),activation='sigmoid'))
        model_sp.add(Dense(10, activation='softmax'))
