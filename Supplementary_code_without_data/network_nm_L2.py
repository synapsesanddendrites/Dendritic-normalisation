...
class L2_layer(Layer):
    ...
    def get_config(self):
        ...
        base_config = super(L2_layer, self).get_config()
...
class L2_SGD(Optimizer):     
    def __init__(self, learning_rate=0.05, **kwargs):
        ...
        super(L2_SGD, self).__init__(**kwargs)
    ...
    def get_updates(self, loss, params):
        ...
        g_delt=K.batch_dot(K.transpose(grmn[0]),K.transpose(K.cast(wght, dtype='float32')),axes=1)/K.expand_dims(n_aff_L2,axis=1)
        term_1=g/n_aff_L2
        term_2=g*K.squeeze(g_delt,axis=1)/K.square(n_aff_L2)
        n_grad=term_1*grmn-term_2*wght
    ...
    def get_config(self):
        ...
        base_config = super(L2_SGD, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
...
model_nm.add(L2_layer(units,kernel_constraint=MaskWeights(non_sparse),activation='sigmoid'))
...
for epoch in range(0,epochs):
    model_nm.compile(loss='sparse_categorical_crossentropy', optimizer=L2_SGD(0.05), metrics=['accuracy'])
    ...
    if epoch<epochs: # Add random new weights if not last run
        ...
        n_aff_L2=np.sqrt(np.sum(np.square(veight),0))
        L2_nm=np.tile(n_aff_L2,[784,1])
        weight=g_vec*veight/L2_nm
        weight[np.isnan(weight)]=0
        wght_vc[0]=weight     
    
        model_nm = Sequential()
        model_nm.add(Flatten(input_shape=(28,28,1)))
        model_nm.add(L2_layer(units,kernel_constraint=MaskWeights(wght_ker),activation='sigmoid'))
        model_nm.add(Dense(10, activation='softmax'))
