...
class Morpho_SGD(Optimizer):     
    ...
    def get_updates(self, loss, params):
        ...
        grads.append(self.get_gradients(loss, params[2]))
        grads.append(self.get_gradients(loss, params[3]))
        grads.append(0)
        grads.append(self.get_gradients(loss, params[5]))
        ...
        mrph_nm=K.transpose(K.tile(n_aff,2880))  
        n_grad=s*grmn/mrph_nm
        s_delt=K.sum(grmn*wght)/s
        
        grads[2]=n_grad
        grads[4]=K.expand_dims(s_delt,axis=0)
...
model_nm = Sequential()
model_nm.add(Conv2D(filters=20, kernel_size=5, activation='sigmoid', input_shape=(28,28,1)))
model_nm.add(MaxPooling2D(pool_size=(2, 2)))
model_nm.add(Flatten())
model_nm.add(Morph_layer(units,kernel_constraint=MaskWeights(non_sparse),activation='sigmoid'))
...
for epoch in range(0,epochs):
    ...
    if epoch<epochs: # Add random new weights if not last run
        ...
        model_nm.add(MaxPooling2D(pool_size=(2, 2)))
        model_nm.add(Flatten())
        model_nm.add(Morph_layer(units,kernel_constraint=MaskWeights(wght_ker),activation='sigmoid'))
        model_nm.add(Dense(10, activation='softmax'))
